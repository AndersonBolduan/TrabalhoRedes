...
def listar_informacoes_do_arquivo_json(diretorio, nome_arquivo, chave_lista):
    ...
    try:
        with open(caminho_arquivo, 'r') as f:
            ...
            return {"status": "sucesso", "arquivos": list(conteudo.keys())}
...
# Função que envia pro cliente e faz o download

def escolher_arquivo_do_json(diretorio, nome_arquivo):
    caminho_arquivo = os.path.join(diretorio, nome_arquivo)

    try: 
        with open(caminho_arquivo, 'r') as f: # Carrega o conteúdo do arquivo JSON
            conteudo = json.load(f)
            return {"status": "sucesso", "arquivos": list(conteudo.keys())}
    except Exception as e:
        return {"status": "erro", "mensagem": str(e)}   

def baixar_arquivo_json(diretorio, nome_arquivo, chave_arquivo): 
    caminho_arquivo = os.path.join(diretorio, nome_arquivo)

    try:
        with open(caminho_arquivo, 'r+') as f:
            conteudo = json.load(f)
            if chave_arquivo in conteudo:
                return {"status": "sucesso", "mensagem": f"Arquivo '{chave_arquivo}' baixado com sucesso."}
            else:
                return {"status": "erro", "mensagem": f"Arquivo '{chave_arquivo}' não encontrado no JSON."}
    except Exception as e:
        return {"status": "erro", "mensagem": str(e)}
...
def processar_requisicao(mensagem):
    try:
        ...
        if "comando" in dados:
            if dados["comando"] == "LISTAR_ARQUIVOS":
            ...
            elif dados["comando"] == "BAIXAR_ARQUIVO":
                # Exemplo: {"comando": "BAIXAR_ARQUIVO", "arquivo": "arquivo1"}
                resposta = baixar_arquivo_json(dados["diretorio"], dados["arquivo"], dados["chave"])
            else:
